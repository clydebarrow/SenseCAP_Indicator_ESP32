...
static int __time_zone_get(char *zone)
{
    esp_tls_cfg_t cfg = {
        .cacert_buf = (const unsigned char *) timeapi_root_cert_pem_start,
        .cacert_bytes = timeapi_root_cert_pem_end - timeapi_root_cert_pem_start,
    ...
    int len  = 0;
    snprintf(time_zone_url, sizeof(time_zone_url),"https://www.timeapi.io/api/TimeZone/zone?timeZone=%s",zone);
    ESP_LOGI(TAG, "TimeZone fetch url: %s", time_zone_url);
